...
- **Новое приглашение** — код в один клик и готовый текст для пересылки.
...
**➕ Новое приглашение** в панели (или `/invite Петров`):

```text
TNVED-7K2M-9XQP
```

Бот сразу даёт готовый текст для пересылки. Человек отправляет `/start TNVED-7K2M-9XQP` и
получает доступ. Код одноразовый, живёт 24 часа, после активации человек появляется в списке.
